...
export default function InstructorStudentsPage() {
  ...
  return (
    <div className="flex gap-6">
            ...
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-white/5">
                {filtered.map((s) => {
                  ...
                  return (
                  <tr key={s.id} className="hover:bg-slate-50 dark:hover:bg-white/4">
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center justify-end gap-2">
                        <Link
                          href={`/instructor/classrooms/${classroomId}/students/${s.user.id}`}
                          className="flex items-center gap-1 rounded border border-slate-200 dark:border-white/10 px-2 py-1 text-xs text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-white/10 hover:text-slate-900 dark:hover:text-white transition-colors"
                        >
                        ...
                        {confirmRemoveId === s.user.id ? (
                          <div className="flex items-center gap-1">
                            ...
                            <button
                              ...
                              className="rounded border border-slate-200 dark:border-white/10 px-2 py-1 text-xs text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white transition-colors"
                            >
                          ...
                          <button
                            ...
                            className="rounded border border-slate-200 dark:border-white/10 p-1 text-slate-500 hover:text-red-500 dark:hover:text-red-400 hover:border-red-500/30 dark:hover:border-red-400/30 transition-colors"
      ...
      {showAddPanel && (
        <aside className="w-80 flex-shrink-0 rounded-xl border border-slate-200 dark:border-white/10 bg-white dark:bg-white/5 p-5 space-y-4 self-start">
          <div className="flex items-center justify-between">
            <h3 className="font-semibold text-slate-900 dark:text-white">{t('instructorClassroomStudents.addPanelTitle')}</h3>
            <button
              type="button"
                ...
                <li key={u.id} className="py-1.5">
                  <label className="flex items-center gap-2 cursor-pointer">
                    <input
                      ...
                      className="rounded border-slate-300 dark:border-white/20 bg-white dark:bg-white/10 text-indigo-600 focus:ring-indigo-500"
                    />
                    <div>
                      <p className="text-sm text-slate-900 dark:text-white">{u.name ?? '—'}</p>
